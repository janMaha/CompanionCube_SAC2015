...
class Spring(object):
    ...
    def __init__(self, axis, axis_history, k, m, c, F, x0, x0_prime):
        ...
        if c == 0 or F == 0:
            ...
            self._method = FreeUnderDamped(k, m, c, x0, x0_prime)
        elif zeta == 1:
            self._method = FreeCriticallyDamped(k, m, c, x0, x0_prime)
        elif zeta > 1:
            self._method = FreeOverDamped(k, m, c, x0, x0_prime)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--x0_prime', type=float, default=-0.1)
    ...
    s = Spring(axis=ax0, axis_history=ax1, k=k, m=PLAT_MASS, c=c, F=F, x0=x0, x0_prime=0)
    cube = IncomingCube(axis=ax0, m=m, x0=XLIMMAX*0.75, x0_prime=x0_prime)

    anim = animation.FuncAnimation(f, s.set_time, interval=delta * 1000, save_count=400)
    anim = animation.FuncAnimation(f, cube.set_time, interval=delta * 1000, save_count=400)
